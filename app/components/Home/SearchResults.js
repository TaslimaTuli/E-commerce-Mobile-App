import { View, Text, StatusBar, TouchableOpacity, ScrollView, SafeAreaView, TextInput, KeyboardAvoidingView, Modal, Platform } from "react-native";
...
export default function SearchResult() {
	const orderStatus = [
		{ name: "Deals", icon: More },
		{ name: "Price", icon: More },
		{ name: "Sort By", icon: More },
		{ name: "Gender", icon: More },
	];
				...
				<ScrollView showsVerticalScrollIndicator={false} keyboardDismissMode="on-drag">
					...
					<ScrollView horizontal showsHorizontalScrollIndicator={false} className="">
						<View className="flex-row space-x-2 px-2 py-1">
							<TouchableOpacity
								className="flex-row items-center justify-between px-3 py-2 rounded-full space-x-1 
									bg-violet-500"
							>
								<Filter />
								<Text className="text-base text-white">2</Text>
							</TouchableOpacity>
							{renderFilterButton("Deals", selectedDeals, "Deals")}
							{renderFilterButton("Price", "Price", "Price")}
					...
					<Modal animationType="slide" transparent visible={showModal} statusBarTranslucent onRequestClose={handleModalClose}>
						<View className="flex-1 justify-end" style={{ backgroundColor: "rgba(0, 0, 0, 0.5)" }}>
							<TouchableOpacity className="flex-1 w-full" activeOpacity={1} onPress={handleModalClose}>
							...
							<View className=" h-[500] bg-white pt-4 px-4 rounded-t-3xl">
								...
								{modalType === "Sort By" && (
									<ScrollView showsVerticalScrollIndicator={false}>
										{sortBy.map((item, index) => (
											...
											>
												<Text className={`text-lg font-extrabold ${item === selectedSortBy ? "text-white" : "text-black"}`}>
													{item}
												</Text>
												<View className="flex-row items-center justify-center space-x-10">
													{item === selectedSortBy ? <Selected /> : <View></View>}
												</View>
											</TouchableOpacity>
										))}
									</ScrollView>
								...
								{modalType === "Gender" && (
									<ScrollView showsVerticalScrollIndicator={false}>
										{gender.map((item, index) => (
											...
											>
												<Text className={`text-lg font-extrabold ${item === selectedGender ? "text-white" : "text-black"}`}>
													{item}
												</Text>
												<View className="flex-row items-center justify-center space-x-10">
													{item === selectedGender ? <Selected /> : <View></View>}
												</View>
											</TouchableOpacity>
										))}
									</ScrollView>
									...
									<ScrollView showsVerticalScrollIndicator={false}>
										{deals.map((item, index) => (
											...
											>
												...
												<View className="flex-row items-center justify-center space-x-10">
													{item === selectedDeals ? <Selected /> : <View></View>}
												</View>
											</TouchableOpacity>
										))}
									</ScrollView>
								...
								{modalType === "Price" && (
									<View>
										<TextInput
											className="bg-stone-200 mx-1 mt-5 py-3 px-5 text-lg text-black rounded-full"
											placeholder="Min"
											numberOfLines={1}
